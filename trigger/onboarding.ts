...
import { sendMessage, getFileUrl, escapeMd } from "../lib/telegram";
...
export async function handleOnboardingStep(
  ...
  switch (step) {
    ...
    case "configure_mcp": {
      // Finalise onboarding when user clicks "Setup Complete"
...
async function uploadDocumentToAnthropic(
  session: UserSession,
  document: NonNullable<TelegramEventPayload["document"]>
): Promise<void> {
...
async function sendMcpInstructions(chatId: number): Promise<void> {
